...
pktgen_packet_classify(struct rte_mbuf *m, int pid)
{
	...
	if (plen == ETHER_MIN_LEN)
		...
		fptr = fopen("/tmp/pktgen_custom.log", "a");
		fprintf(fptr, "Runt packet\n");
		if (pType == ETHER_TYPE_ARP)
